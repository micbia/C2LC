...
# ----------------------------------------------------------------------------------------
path_in = '/work/ska/HIRAX/UchuuSim/Uchuu/RockstarExtended/MHI_Modi2019_ModelC/'
path_out = path_in
dthet, dnu = 0.5, 5. # in deg and MHz
interpolation = 'sigmoid'
# ----------------------------------------------------------------------------------------

typ_model = path_in[path_in[:-1].rfind('/')+1:-1]
fout_lc = '%slc_dT_%s_%sdeg_%dMHz.npy' %(path_out, typ_model, str(dthet), dnu)
...
for i_z0, z0 in zip(idx_z, cube_redshift):
    # get HI mass
    mHI = np.load('%slc_z%d_0.5deg_5MHz.npy' %(path_out, i_z0)) * u.Msun
    ...
    np.save('%sdT_z%.2f_%sdeg_%dMHz.npy' %(path_out, z0, str(dthet), dnu), dT.value) # in mK

mesh_size = mHI.shape
######## Make lightcone ########

comoving_pos_idx = 0
z_bracket_low, z_bracket_high = None, None
data_low, data_high = None, None

assert lc_redshift.size == mesh_size[0]
lightcone = np.zeros(mesh_size, dtype='float32')
...
for i_l in tqdm(range(lc_redshift.size)):
    ...
    data_low = np.load('%sdT_z%.2f_%sdeg_%dMHz.npy' %(path_out, z_low, str(dthet), dnu), mmap_mode='r')
    data_high = np.load('%sdT_z%.2f_%sdeg_%dMHz.npy' %(path_out, z_high, str(dthet), dnu), mmap_mode='r')
